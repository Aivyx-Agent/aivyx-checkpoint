...
aivyx-checkpoint is a library, not a standalone application — it
provides git-ref checkpoint/rollback for agent tool calls, consumed by
`aivyx` and `aivyx-coder` as a dependency. A vulnerability here is
usually most consequential in how it affects one of those two
consuming projects, and should still be reported.
...
Advisories](https://github.com/Aivyx-Agent/aivyx-checkpoint/security/advisories/new)
